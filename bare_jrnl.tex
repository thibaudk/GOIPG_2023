...
At the outset, it is our belief that common practices on hardware interfaces, consumer preferences and comparative analysis of interactive systems is already the object of previous research. As such, we consider selecting for a significant population sample, conducting interviews, collecting and analysing user data to be outside of the scope of this research. Based on this existing academic literature \cite{rossmy:grid} \cite{pust:taxonomy}, as well as commercial product reviews, we will first aim to establish a set of basic representation elements, compatible with our devices of choice. This first exploratory phase will also necessitate delving into theoretical aspects of temporal models \cite{milliere:topologie} \cite{toro:condition} to ensure that our basic elements are suited to a variety of time representations. We should then be able to move from representation, to control, and finally to edition of temporal models. At every step of this progression, we hope to offer partial answers to our problematic, firstly put in most general of terms: "How can we interact with open ended, abstract software on finite physical interfaces ?" This question is particularly delicate with software straying far away from the norm, and hardware interfaces incarnating conventions in musical expression. Such a set of generic temporal elements can already be found in the branching timeline model for example  \cite{jcelerier:thesis}. It can be summarized by:
\begin{itemize}
    ...
    \item \textit{Intervals}: flexible duration connecting two distinct \textit{States}
    \item \textit{Scenarios}: spanning the duration of an \textit{Interval}, it can contain all other elements
\end{itemize}
While this reduced set may suffice to represent interactive scores, additional elements are are required to provide basic control:
\begin{itemize}
...
\end{itemize}
Are more elements required to complete our progression from representation and control to edition ? 
How can his set be restricted ?
How can tradition hardware sequencing be defined with a similar set ?
after defining Defining our set of generic elements, they could then be reconsidered in the specific context of musical time and instrumental performance. 
Edition is the part we claim to be the innovative part of this research. 
How can musicians rewrite a score as they are playing it ?
...
Human trials necessary.
extracting relevant and reliable data from a dense market

\subsection{Contingency plans}
If our initial assessment reveals itself to be lacking, i.e if we do find out that human trains are necessary, we reserve the rite to reshape our schedule and 
